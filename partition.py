    ...
    **Caution**: modifies ``node`` (passed by reference).
    ...
    if not oracle.P_theta(theta=c_R,check_feasibility=True):
        ...
        if delta_hat is None:
            ...
            vertex_costs = np.array([oracle.P_theta_delta(theta=vertex,delta=delta_hat)[1]
...
def lcss(oracle,node,eps_a,eps_r,rho_max,animator=None,progressbar=None):
    """
    ...
    * Assuming that no leaf is closed due to a bad condition number.
    ...
    def add(child_left,child_right):
        ...
        for child in [node.left,node.right]:
            ...
            if rho > rho_max:
                ...
                # Add open leaf
                lcss(oracle,child,eps_a,eps_r,rho_max,animator,progressbar)
                
    def update_vertex_costs(v_mid,v_combo_idx,delta,old_vertex_costs):
        """
        ...
        """
        V_delta_v_mid = oracle.P_theta_delta(theta=v_mid,delta=delta)[1]
        ...
        return vertex_costs_S_1,vertex_costs_S_2
    ...
    eps_suboptimal = bar_e_a_R<=eps_a or bar_e_r_R<=eps_r
    if infeasible or eps_suboptimal:
        ...
        if D_delta_R_feasible:
            new_vertex_costs = np.array([oracle.P_theta_delta(theta=vertex,delta=delta_star)[1]
                                         for vertex in node.data.vertices])
            if oracle.in_variability_ball(R=node.data.vertices,
                ...
                lcss(oracle,node,eps_a,eps_r,rho_max,animator,progressbar)
            else:
                ...
                vertex_costs_S_1,vertex_costs_S_2 = update_vertex_costs(
                        v_mid,v_idx,delta_star,new_vertex_costs)
                # Make children
            ...
            v_mid = S_1[v_idx[0]]
            vertex_costs_S_1,vertex_costs_S_2 = update_vertex_costs(
                    v_mid,v_idx,node.data.commutation,node.data.vertex_costs)
            # Make children
            child_left = NodeData(vertices=S_1,
